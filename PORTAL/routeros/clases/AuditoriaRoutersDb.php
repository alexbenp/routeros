...
class AuditoriaRoutersDb extends Conexion {
	...
	public function getAcciones($accion){
		...
		try {
			...
			IF(EMPTY($accion)){
				$sql = $this->conexion->prepare('SELECT accion_id, accion FROM acciones ORDER BY accion_id' );
				$sql->execute();
				$resultado = $sql->fetchAll();
				$this->codigoRespuesta = "00";
				$this->mensajeRespuesta = "Resultado Consulta Exitoso: ";
			}ELSE{
				$sql = $this->conexion->prepare('SELECT accion_id, accion FROM acciones WHERE accion = :accion ORDER BY accion_id DESC' );
	...
	public function ipHotspotUserAddDB($accion_id,$id_usuario_router,$usuario_router,$clave_usuario_router,$tiempo_unidad,$perfil_usuario_router,$comentario_registro){
		...
		try {
			...
			if($this->router_id > 0){
				$sql = $this->conexion->prepare('INSERT INTO '. self::TABLA .' (router_id, usuario_id, accion_id,id_usuario_router, usuario_router, clave_usuario_router, tiempo_unidad, perfil_usuario_router, comentario_registro) VALUES (:router_id, :usuario_id, :accion_id, :id_usuario_router, :usuario_router, :clave_usuario_router, :tiempo_unidad, :perfil_usuario_router, :comentario_registro)' );
	...
	public function ipHotspotUserAddDBUpdate($auditoria_registro_id,$id_usuario_router,$respuesta){
		...
		try {
			...
			if($auditoria_registro_id > 0){
				$sql = $this->conexion->prepare('UPDATE '. self::TABLA .' SET id_usuario_router = :id_usuario_router, respuesta = :respuesta WHERE auditoria_registro_id = :auditoria_registro_id;' );
				$sql->bindParam(':id_usuario_router', $id_usuario_router);
	...
	public function ipHotspotUserProfileAddDB($accion_id,$profile_name,$user_shared,$rx,$tx,$add_mac_cookie,$mac_uptime){
		...
		try {
			...
			$this->mensajeRespuesta = "Reg Audit: ";
			if($this->router_id > 0){
				$sql = $this->conexion->prepare('INSERT INTO auditoria_registro_perfil_router (router_id, usuario_id, accion_id, profile_name, user_shared, rx, tx, add_mac_cookie,mac_uptime) VALUES (:router_id, :usuario_id, :accion_id, :profile_name, :user_shared, :rx, :tx, :add_mac_cookie, :mac_uptime)' );
				$sql->bindParam(':router_id', $this->router_id);
				$sql->bindParam(':usuario_id', $this->usuario_id);
				$sql->bindParam(':accion_id', $accion_id);
	...
	public function ipHotspotUserProfileAddDBUpdate($auditoria_registro_perfil_id,$id_perfil_usuario_router,$respuesta){
		...
		try {
			...
			if($auditoria_registro_perfil_id > 0){
				$sql = $this->conexion->prepare('UPDATE auditoria_registro_perfil_router SET id_perfil_usuario = :id_perfil_usuario_router, respuesta = :respuesta WHERE auditoria_registro_perfil_id = :auditoria_registro_perfil_id;' );
